...
class Project:
  ...
  def _append_item_to_project(self, item):
    ...
    closed = str(item.status.text) in ('Done', 'Closed')
    closed_at = ''
    if closed:
      try:
  ...
  def _add_relationships(self, item):
    ...
    skiplist=["tested by","is blocked by","is cloned by","relates to"]
    try:	  
      for issuelinktype in item.issuelinks.issuelinktype:
        for inwardlink in issuelinktype.inwardlinks:
          if inwardlink.get("description") not in skiplist:
